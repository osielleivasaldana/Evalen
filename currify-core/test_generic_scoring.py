...
async def test_generic_scoring():
    try:
        ...
        if response:
            ...
            # Helper to print nested breakdown safely
            def print_dim(name, key):
                score = response.breakdown.get(key, {}).score if response.breakdown.get(key) else 0
                print(f"{name} Score: {score}")
                return score

            exp_score = print_dim("Experience", "experience")
            skills_score = print_dim("Skills", "skills_match")
            
            if exp_score > 70 and skills_score > 70:
                print("\n🎉 SUCCESS: AI correctly normalized roles and extracted latent skills!")
            else:
                print("\n⚠️ PARTIAL/FAILURE: Scores are too low.")
                print(f"Reasoning Exp: {response.breakdown.get('experience', {}).reasoning}")
                print(f"Reasoning Skills: {response.breakdown.get('skills_match', {}).reasoning}")
        else:
